...
-   我们至少应该为试验中用到的超参数自动生成基本超参数轴图。
-   此外，我们会自动为所有试验生成训练曲线，并确保我们能够很轻松的找到每次研究的最佳试验和对应试验的训练曲线。
-   除此之外，还有许多其他有用的图表和可视化方法。上面描述的图表是很好的起点，但套用杰弗里 · 辛顿 (Geoffrey Hinton) 的话，“每当你绘制新事物时，你都会学到一些新知识。”
...
-   当我们迭代模型时，我们可能会发现与现有配置相比，某个候选最初有更好的验证错误率，但在重复实验后并没有持续的优势。大致上，我们可以将导致这种不一致的最重要原因分为以下几大类：
    -   **培训流程差异**、**重新训练差异**或**试验差异**：源自同样超参数配置下，使用不同随机种子带来的差异。
        -   例如，不同的随机初始化、训练数据打乱、随机失活、数据扩充操作的模式以及并行计算操作的顺序，都是试验差异的潜在来源。
...
-   我们可以使用严格的统计测试在有限验证集上估算验证错误率，但通常试验差异本身就可以在使用相同超参数设置的两个不同训练之间产生统计显著差异。
-   当我们试图得出超出超参数空间中单个点水平的结论时，我们最关心的是研究差异。
    -   研究差异取决于试验次数和搜索空间，我们遇到过它大于试验方差的情况以及它小得多的情况。
-   因此，在采用候选更改之前，请考虑将最佳试验运行 N 次以了解每次运行的试验差异。
    -   通常，我们可以仅在对训练流水线进行了重大变更后再测量试验差异，但在某些应用程序中，我们可能需要更频繁的估计试验差异。
    -   在其他应用程序中，测量试验差异的成本太高所以不值得做。
...
-   此时，我们还应该考虑检查测试集上的性能。
    -   原则上，我们甚至可以将验证集放到训练集中，并重新训练通过贝叶斯优化找到的最佳配置。但是，这仅适用于未来不会再次上线的情况（例如，一次性 Kaggle 竞赛）。
